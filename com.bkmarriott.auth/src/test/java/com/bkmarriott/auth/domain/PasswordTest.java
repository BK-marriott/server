...
@DisplayName("[Domain] Password Unit Test")
class PasswordTest {

    @Test
    @DisplayName("[비밀번호 생성 성공 테스트] 암호화를 진행하지 않고 Password 객체를 생성한다.")
    void passwordGenerate_successTest_notEncrypting() {
        // Given
        String originPassword = "password";
        Password password = Password.valueOf(originPassword);
        // When & Then
        Assertions.assertAll(
            () -> Assertions.assertEquals(originPassword, password.getEncryptedPassword())
        );
    }

    @Test
    @DisplayName("[비밀번호 생성 성공 테스트] 암호화를 진행한 뒤 Password 객체를 생성한다.")
    void passwordGenerate_successTest_withEncrypting() {
        // Given
        String originPassword = "password";
        Password password = Password.generateWithEncrypting(originPassword);
        // When & Then
        Assertions.assertAll(
            () -> Assertions.assertNotEquals(originPassword, password.getEncryptedPassword())
        );
    }

    @Test
    @DisplayName("[비밀번호 생성 실패 테스트] 생성자로 빈 문자열이 주어지는 경우 예외를 발생한다.")
    void passwordGenerate_failureTest_givenBlankPassword() {
        // Given
    ...
    void passwordMatching_successTest() {
        // Given
        String originPassword = "password";
        Password password = Password.generateWithEncrypting(originPassword);
        // When
    ...
    void passwordMatching_failureTest() {
        // Given
        String originPassword = "password";
